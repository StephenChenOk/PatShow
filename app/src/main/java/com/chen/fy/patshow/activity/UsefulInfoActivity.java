...
public class UsefulInfoActivity extends AppCompatActivity implements RadioGroup.OnCheckedChangeListener {
    ...
    public void onWindowFocusChanged(boolean hasFocus) {
        ...
        mIntroductionBoxHeight = llIntroductionBox.getMeasuredHeight();
        mTravelNotesBoxHeight = llTravelNotesBox.getMeasuredHeight();
        mStrategyBoxHeight = llStrategyBox.getMeasuredHeight();
    }
    ...
    private void bindView() {
        ...
        mNestedScrollView.setOnScrollChangeListener(new NestedScrollView.OnScrollChangeListener() {
            ...
            public void onScrollChange(NestedScrollView v, int scrollX, int scrollY, int oldScrollX, int oldScrollY) {
                if(!isClickBtn) {
                    if (scrollY <= mIntroductionBoxHeight) {
                        rbIntroduction.setChecked(true);
                    } else if (scrollY <= mTravelNotesBoxHeight) {
    ...
    public void onCheckedChanged(RadioGroup group, int checkedId) {
        ...
        mHandler.sendEmptyMessageAtTime(0,INTERVAL_TIME);
    }
}
